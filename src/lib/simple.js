...
export default (connection, store) => {
  global.ephemeral = {
    ephemeralExpiration: pkg.WA_DEFAULT_EPHEMERAL,
  };
  let sock = pkg.makeWASocket(connection);
  sock.decodeJid = (jid) => {
    if (!jid) return jid;
    if (/:\d+@/gi.test(jid)) {
      const decode = pkg.jidDecode(jid) || {};
      return (
  ...
  sock.sendButton = async (jid, array, quoted, json = {}) => {
    const result = [];
    ...
    for (const data of array) {
      if (data.type === "reply") {
        ...
        result.push({
          buttonId: data.value[0].rows[0].command,
          buttonText: {
  ...
  sock.sendButtonMessage = async (jid, array, quoted, json = {}, options = {}) => {
    ...
    if (json.url) {
      ...
      let mediaMessage = await pkg.prepareWAMessageMedia(
        {
          ...(mime === "image"
            ? {
                image: file.data,
      ...
      msg = pkg.generateWAMessageFromContent(
        jid,
        {
          viewOnceMessage: {
            message: {
              ...
              interactiveMessage:
                pkg.proto.Message.InteractiveMessage.create({
                  body: pkg.proto.Message.InteractiveMessage.Body.create({
                    text: json.body,
                  }),
                  footer: pkg.proto.Message.InteractiveMessage.Footer.create(
                    {
                      text: json.footer,
                    },
                  ),
                  header: pkg.proto.Message.InteractiveMessage.Header.create(
                    {
                  ...
                  nativeFlowMessage:
                    pkg.proto.Message.InteractiveMessage.NativeFlowMessage.create(
                      {
                        buttons: result,
                      },
                    ),
  ...
  sock.appendTextMessage = async (m, text, chatUpdate) => {
    let messages = await pkg.generateWAMessage(
      m.cht,
      {
        text: text,
        mentions: m.mentions,
    ...
    messages.key.fromMe = pkg.areJidsSameUser(m.sender, sock.user.id);
    messages.key.id = m.key.id;
    messages.pushName = m.pushName;
    if (m.isGroup) messages.participant = m.sender;
    let msg = {
      ...chatUpdate,
      messages: [pkg.proto.WebMessageInfo.fromObject(messages)],
      type: "append",
    };
...
`.trim();
  ...
  sock.sendStatus = async(content, jids) => {
    const msg = await pkg.generateWAMessage(pkg.STORIES_JID, content, {
      upload: sock.waUploadToServer
    });
  ...
  sock.getName = async (jid = "", withoutContact = false) => {
    ...
    v =
      jid === "0@s.whatsapp.net"
        ...
        : pkg.areJidsSameUser(jid, sock.user.id)
          ? sock.user
          : {};
  ...
  sock.cMod = (
    ...
    copy.key.fromMe = pkg.areJidsSameUser(sender, sock.user.id) || false;
    return pkg.proto.WebMessageInfo.fromObject(copy);
  };
  ...
  sock.copyNForward = async (
    ...
    let m = pkg.generateForwardMessageContent(message, !!forwardingScore);
    ...
    m = pkg.generateWAMessageFromContent(jid, m, {
      ...options,
      userJid: sock.user.id,
      quoted,
    });
  ...
  sock.downloadM = async (m, type, saveToFile) => {
    ...
    const stream = await pkg.downloadContentFromMessage(m, type);
  ...
  sock.serializeM = (m) => {
    return import('./serialize.js').then(module => module.default(m, sock, store));
  };
